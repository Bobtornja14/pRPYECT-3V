...
def list():
    ...
    if search:
        query = query.filter(Parte.nombre.contains(search) |
                             Parte.codigo.contains(search))

    # Asegúrate de que el filtro de stock bajo funciona correctamente
    if stock_bajo:
        query = query.filter(Parte.stock <= Parte.stock_minimo)

    # Si tienes un campo 'activo', asegúrate de filtrar solo activos si corresponde
    # query = query.filter_by(activo=True)  # Descomenta si usas este campo

    partes = query.order_by(Parte.nombre).paginate(
        page=page, per_page=10, error_out=False)

    # Contar partes con stock bajo
    stock_bajo_count = Parte.query.filter(Parte.stock <= Parte.stock_minimo).count()
...
def delete(id):
    ...
    if parte.pedidos:
        flash('No se puede eliminar la parte porque tiene pedidos asociados', 'error')
        return redirect(url_for('partes.list'))
