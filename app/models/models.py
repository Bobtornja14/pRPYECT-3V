...
class Usuario(UserMixin, db.Model):
    ...
    rol = db.Column(db.String(20), nullable=False, default='administrador')  # Solo admin o tecnico
...
class Cliente(db.Model):
    ...
    activo = db.Column(db.Boolean, default=True)  # <-- Agrega esta línea
...
class Asignacion(db.Model):
    ...
    # Relaciones
    reportes = db.relationship('Reporte', backref='asignacion', lazy=True)
    pedidos_piezas = db.relationship('PedidoPieza', backref='asignacion', lazy=True)
...
class PedidoPieza(db.Model):
    ...
    estado = db.Column(db.String(20), default='pendiente')  # <-- Default pendiente
...
# (Verifica que todas las relaciones y claves foráneas estén correctas)
