...
def run_migrations():
    ...
    try:
        ...
        # Asociar técnicos existentes con usuarios de rol 'tecnico'
...
def add_usuario_id_to_tecnicos():
    """Agrega la columna usuario_id a tecnicos si no existe."""
    # Usar la ruta de la base de datos desde la configuración
    from app import db
    db_path = db.engine.url.database
    if not os.path.exists(db_path):
        print("No existe la base de datos, crea primero las tablas con db.create_all()")
        return
    import sqlite3
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    ...
    if 'usuario_id' not in columns:
        ...
        print("La columna usuario_id ya existe en tecnicos.")
    conn.close()
